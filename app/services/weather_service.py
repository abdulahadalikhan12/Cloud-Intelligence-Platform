...
async def get_weather_history(city: str, lat: float, lon: float, days: int = 30) -> WeatherHistory:
    ...
    params = {
        ...
        "daily": [
            ...
            "rain_sum", "wind_speed_10m_max", "surface_pressure_mean",
        ],
    }
    ...
    daily_data = []
    for i in range(len(daily["time"])):
        daily_data.append(HistoricalWeather(
            date=daily["time"][i],
            ...
            pressure_hpa=daily.get("surface_pressure_mean", [None])[i] if i < len(daily.get("surface_pressure_mean", [])) else None,
        ))
